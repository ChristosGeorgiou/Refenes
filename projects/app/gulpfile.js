...
gulp.task('build', ['build:sass', 'build:client', 'generate:nglb']);

gulp.task('watch', ['build:sass', 'build:client', 'generate:nglb'], function() {

    gulp.watch('./scss/**/**.*', ['build:sass']);
    gulp.watch('./client/src/**/*.js', ['build:client']);
    gulp.watch('./common/models/**/*.*', ['generate:nglb']);

});

gulp.task('generate:nglb', shell.task([
...
gulp.task('build:client', ['build:client-config'], function() {
...
gulp.task('build:client-config', function() {
    ...
    app.models.Role.create(_roles, function(err, roles) {
        ...
        roles.forEach(function(role, i) {
            ...
            if (_auth.users.length === 0) {
                ...
                gutil.log('Empty role created',
                    gutil.colors.magenta(role.name));
                return;
            }
            ...
            app.models.AppUser.create(_users, function(err, users) {
                ...
                users.forEach(function(user, j) {
                    ...
                    role.principals.create({
                        ...
                        if (err) cb(err);
                        gutil.log('Created user',
                            gutil.colors.magenta(user.username),
                            '[', gutil.colors.magenta(role.name), ']');
                    });
                });
            });
...
gulp.task('db:migrate-schema', function(cb) {
    ...
    var models = require(
        path.resolve(__dirname, './server/model-config.json'));
    var datasources = require(
        path.resolve(__dirname, './server/datasources.json'));
...
gulp.task('db:mock', ['db:clean'], function(cb) {
    ...
    glob('./mock/*.json', {}, function(err, files) {
        ...
        _.each(_models, function(_model, i) {
            ...
            gutil.log('Inserting',
                gutil.colors.magenta(_model.data.length),
                'items of',
                gutil.colors.magenta(_model.name));
